...
public class SqueletaZEntity extends Skeleton implements IAnimatable {
	...
	public static final EntityDataAccessor<String> ANIMATION = SynchedEntityData.defineId(SqueletaZEntity.class, EntityDataSerializers.STRING);
	public static final EntityDataAccessor<String> TEXTURE = SynchedEntityData.defineId(SqueletaZEntity.class, EntityDataSerializers.STRING);
	...
	public SqueletaZEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(AzmodeModEntities.SQUELETA_Z.get(), world);
	}
	...
	protected void defineSynchedData() {
		...
		this.entityData.define(TEXTURE, "strayed_one");
	}
	...
	public static void init() {
		SpawnPlacements.register(AzmodeModEntities.SQUELETA_Z.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
				(entityType, world, reason, pos, random) -> (world.getDifficulty() != Difficulty.PEACEFUL && Monster.isDarkEnoughToSpawn(world, pos, random) && Mob.checkMobSpawnRules(entityType, world, reason, pos, random)));
		DungeonHooks.addDungeonMob(AzmodeModEntities.SQUELETA_Z.get(), 180);
	}
	...
	private <E extends IAnimatable> PlayState movementPredicate(AnimationEvent<E> event) {
		if (this.animationprocedure.equals("empty")) {
			...
			if (this.isInWaterOrBubble()) {
				event.getController().setAnimation(new AnimationBuilder().addAnimation("swing", EDefaultLoopTypes.LOOP));
				return PlayState.CONTINUE;
			}
	...
	protected void tickDeath() {
		...
		if (this.deathTime == 15) {
			this.remove(SqueletaZEntity.RemovalReason.KILLED);
			this.dropExperience();
			SqueletaZLorsqueLeTempsDeMortEstAtteintProcedure.execute(this.level, this.getX(), this.getY(), this.getZ(), this);
		}
	}
